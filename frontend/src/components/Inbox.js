...
function Inbox() {
    ...
    const VALENTINESDAY = (dateToday.getMonth() === 2 && dateToday.getDate() === 14);
    ...
    const handleSubmit = async (e) => {
        ...
        if ((emailFilter === "admin" && passwordFilter === "Admin_123") || userSelected){
            ...
            const newFilteredItems = items.filter(
                (item) =>
                    (item.recipient?.email === emailFilter && item.recipient?.password === passwordFilter) || item.sender === "TEAM NAME" //TODO ADD TEAM NAME OR WHATEVER
            );
            if(VALENTINESDAY){
